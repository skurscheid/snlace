"""Clustering of study species genome-guided and related species
   superTranscriptomes, and de novo assembly -> now multithreaded
...
rule blat_relST_genomeST:
    ...
    params:
        options = config["params"]["blat_related"],
        pblat = "/home/sarah/packages/misc/pblat/icebert-pblat-e05e284/pblat"
    threads: 1 #config["max_threads"]/3
    benchmark: CLUSTER_OUTDIR + "07blat_relST_gST.times.tab"
    output:
        CLUSTER_OUTDIR + "relST_genomeST.psl"
    shell:
        "{params.pblat} -threads={threads} {params.options} {input.related} {input.genome} {output} "
